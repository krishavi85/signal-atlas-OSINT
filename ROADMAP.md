...
| User-provided URL ingestion (§3) | ✅ the web-generic connector's `fetch()` existed since Phase 1 (SSRF-guarded, robots-aware, cached) but had no caller anywhere — found and fixed while wiring up Playwright. `POST /projects/:id/evidence/fetch-url` (Evidence-tab UI) → one Evidence record → entities → relationship/claim/timeline passes, same shape as document ingestion. Optional `render: true` renders the page's JS in headless Chromium first (`packages/connectors/src/lib/browserFetch.ts`) for pages a plain fetch can't see content on — replicates the SSRF guard independently at the browser layer (Playwright bypasses `safeFetch` entirely) by validating the initial URL and intercepting every subresource/redirect/in-page request the render triggers. Deliberately does not click, fill forms, or log in — rendering a page's own JS stays in scope, automating access-controlled surfaces doesn't (§30). Verified live: a plain fetch of a Wikipedia article extracted 60 real entities; a JS-rendered fetch of a page that only populates content via JavaScript captured the actual rendered quotes/text; a private-IP target was correctly refused with `IPv4 127.0.0.1 is in a blocked range` |
...
| Phone OSINT (libphonenumber, numverify optional) | ✅ | `PhoneLookup.ts` — offline parse/validate/type via `libphonenumber-js/max` (Google's own metadata); live carrier/line-type/location via numverify when `NUMVERIFY_API_KEY` is set, else a named capability gap. Dork-query links generated alongside every result. |
| Corporate OSINT (OpenCorporates, SEC EDGAR, Companies House, USPTO) | ✅ | `CorporateLookup.ts`. SEC EDGAR always live, keyless: exact registrant match resolved from SEC's own ticker/CIK registry (not full-text search, which only proves a name was *mentioned* somewhere — that's reported separately as "mentions" so the two are never conflated). OpenCorporates verified to now require a registered API token (anonymous search returns "Invalid Api Token") — gated behind `OPENCORPORATES_API_KEY`. Companies House gated behind `COMPANIES_HOUSE_API_KEY` (Basic Auth). USPTO: no stable documented name-search API was found, so it's a launcher link into USPTO's own tool rather than a guessed-at endpoint. |
| Geospatial (Nominatim, WiGLE, OpenSky) | ✅ | `GeoLookup.ts`. Nominatim geocode/reverse-geocode live and keyless, but its usage policy actively blocks many hosting/cloud IP ranges — a block is reported as exactly that (HTML-instead-of-JSON detected), not swallowed. OpenSky flight-state lookup works fully anonymously (verified live). WiGLE WiFi lookup gated behind `WIGLE_API_NAME`/`WIGLE_API_TOKEN` (a real account is required for any lookup at all, verified). |
| Threat intel (VirusTotal/URLScan/OTX, optional keys) | ✅ | `ThreatIntelLookup.ts`. URLScan.io *search* of already-submitted scans verified keyless-live; VirusTotal and AlienVault OTX both require a free-tier key even for basic lookups (verified), gated behind `VIRUSTOTAL_API_KEY`/`OTX_API_KEY`. Report URLs can be promoted into evidence. |
| Dork/search toolkit + reverse-image/PimEyes launcher links | ✅ | `packages/core/src/dorks.ts` (pure, unit-tested) builds labelled dork queries per target type (domain/email/phone/username/generic) and cross-engine links (Google/Bing/DuckDuckGo/Yandex) — the platform builds queries for the analyst to run themselves, it never automates a query against a search engine's own UI. Reverse-image-search launcher links (Google Lens/Yandex/Bing/TinEye) take a public image URL; PimEyes has no URL-based deep link (upload-only in their UI), so that's an honest launcher to PimEyes itself, not a fabricated pre-filled search. `grep.app`'s public-looking search API was evaluated and dropped — live-tested and found sitting behind a Vercel bot-detection challenge page from automated requests, which is exactly the anti-bot surface this platform won't try to defeat (§30); GitHub's own official code-search API (already used by the GitHub connector's `GITHUB_TOKEN`) covers the same need lawfully. |
